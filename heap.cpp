...
void MaxHeap::insert(int value, int priority) {
    // allow duplicates: always push and update index_map_
...
void MaxHeap::change_priority(int value, int new_priority) {
    ...
    if (it == index_map_.end()) throw std::invalid_argument("Value not found");
    std::size_t idx = it->second;
    int old_priority = heap_[idx].priority;
    heap_[idx].priority = new_priority;
    if (new_priority > old_priority) sift_up(idx);
    else if (new_priority < old_priority) sift_down(idx);
}
